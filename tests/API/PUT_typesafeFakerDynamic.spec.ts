...
test('Typesafe, Faker, dynamic - PUT API with Token ', async ({ request }) => {
    ...
    //put api
    const putres = await request.put('/booking/' + bid, {
        headers: {
    ...
    expect(putres.statusText()).toBe('OK');
    const jputres = await putres.json();
    console.log('PUT Response is' + JSON.stringify(jputres, null, 2));
});